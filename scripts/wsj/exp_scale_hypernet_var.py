...
def set_network_trainer(input_data,
    ...
    for inner_hid in inner_hid_list:
        ...
        train_var_cost += T.mean(seq_var)

    train_var_cost /= num_inners

    # # increase inner loop mean difference (over time)
    # train_mean_cost = 0.
    # for inner_hid in inner_hid_list:
    #     # mean over time
    ...
    #
    #     # var over sample
    #     train_mean_cost -= T.mean(T.var(input=seq_mean, axis=0))

    # get network parameters
    network_params = get_all_params(network, trainable=True)

    # get network gradients with clipping
    network_grads = theano.grad(cost=train_predict_cost + train_regularizer_cost*l2_lambda + train_var_cost*var_lambda,
                                wrt=network_params)
        ...
        for e_idx in range(options['num_epochs']):
            ...
            for b_idx, data in enumerate(train_datastream.get_epoch_iterator()):
                ...
                if total_batch_cnt%options['train_disp_freq'] == 0 and total_batch_cnt!=0:
                    ...
                    print 'Variance Cost: ', str(train_var_cost)
