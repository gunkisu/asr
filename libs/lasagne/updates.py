...
def sgd(loss_or_grads, params, learning_rate, load_params_dict=None):
    ...
    * ``param := param - learning_rate * gradient``
    ...
    updates = OrderedDict()
    opt_params_dict = OrderedDict()

    for param, grad in zip(params, grads):
        updates[param] = param - learning_rate * grad

    return updates, opt_params_dict


def apply_momentum(updates, params=None, momentum=0.9, load_params_dict=None):
    ...
    * ``param := param + velocity``
    ...
    for param in params:
        ...
        if load_params_dict:
            velocity = theano.shared(load_params_dict['velocity'],
            ...
            velocity = theano.shared(np.zeros(value.shape, dtype=value.dtype),
                                     broadcastable=param.broadcastable,
                                     name='velocity')
        opt_params_dict['velocity'] = velocity
...
def momentum(loss_or_grads, params, learning_rate, momentum=0.9, load_params_dict=None):
    ...
    * ``param := param + velocity``
    ...
    """
    updates, _ = sgd(loss_or_grads, params, learning_rate, load_params_dict)
    return apply_momentum(updates, momentum=momentum, load_params_dict=load_params_dict)


def apply_nesterov_momentum(updates, params=None, momentum=0.9, load_params_dict=None):
...
def nesterov_momentum(loss_or_grads, params, learning_rate, momentum=0.9, load_params_dict=None):
    ...
    * ``param := param + momentum * velocity - learning_rate * gradient``
    ...
    """
    updates, _ = sgd(loss_or_grads, params, learning_rate, load_params_dict)
    return apply_nesterov_momentum(updates, momentum=momentum, load_params_dict=load_params_dict)


def adagrad(loss_or_grads, params, learning_rate=1.0, epsilon=1e-6, load_params_dict=None):
    """Adagrad updates
    ...
    for param, grad in zip(params, grads):
        value = param.get_value(borrow=True)
        if load_params_dict:
            accu = theano.shared(load_params_dict['accu'],
                                 broadcastable=param.broadcastable,
                                 name='accu')
        else:
...
def rmsprop(loss_or_grads, params, learning_rate=1.0, rho=0.9, epsilon=1e-6, load_params_dict=None):
    """RMSProp updates
...
def adadelta(loss_or_grads, params, learning_rate=1.0, rho=0.95, epsilon=1e-6, load_params_dict=None):
    """ Adadelta updates
    ...
    for param, grad in zip(params, grads):
        ...
        if load_params_dict:
            ...
            delta_accu = theano.shared(load_params_dict['delta_accu'],
                                       broadcastable=param.broadcastable,
                                       name='delta_accu')
        else:
            accu = theano.shared(np.zeros(value.shape, dtype=value.dtype),
            ...
            delta_accu = theano.shared(np.zeros(value.shape, dtype=value.dtype),
                                       ...
                                       name='delta_accu')
        opt_params_dict['accu'] = accu
        opt_params_dict['delta_accu'] = delta_accu
...
def adam(loss_or_grads, params, learning_rate=0.001, beta1=0.9,
         beta2=0.999, epsilon=1e-8, load_params_dict=None):
    """Adam updates
    ...
    if load_params_dict:
        t_prev = theano.shared(load_params_dict['t_prev'],
                               name='t_prev')
    else:
        t_prev = theano.shared(utils.floatX(0.),
                               name='t_prev')
    opt_params_dict['t_prev'] = t_prev
    ...
    for param, g_t in zip(params, all_grads):
        value = param.get_value(borrow=True)
        if load_params_dict:
            m_prev = theano.shared(load_params_dict['m_prev'],
                                   broadcastable=param.broadcastable,
                                   name='m_prev')
            v_prev = theano.shared(load_params_dict['v_prev'],
                                   broadcastable=param.broadcastable,
                                   name='v_prev')
        else:
            ...
            v_prev = theano.shared(np.zeros(value.shape, dtype=value.dtype),
                                   broadcastable=param.broadcastable,
                                   name='v_prev')
        opt_params_dict['m_prev'] = m_prev
        opt_params_dict['v_prev'] = v_prev
    ...
    for param, g_t in zip(params, all_grads):
        ...
        if load_params_dict:
            m_prev = theano.shared(load_params_dict['m_prev'],
                                   broadcastable=param.broadcastable,
                                   name='m_prev')
            u_prev = theano.shared(load_params_dict['u_prev'],
                                   broadcastable=param.broadcastable,
                                   name='u_prev')
        else:
            m_prev = theano.shared(np.zeros(value.shape, dtype=value.dtype),
