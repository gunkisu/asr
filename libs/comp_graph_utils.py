...
def trainer(input_data, input_mask, target_data, target_mask, network, updater, 
        learning_rate, delay=0, load_updater_params=None, ivector_data=None):
...
def predictor(input_data, input_mask, target_data, target_mask, network, delay=0, ivector_data=None):
...
def eval_net_skip(predict_fn, data_stream, skip, skip_random, use_ivectors=False):
    ...
    for batch_cnt, data in enumerate(data_iterator, start=1):
        ...
        if use_ivectors:
            predict_output = predict_fn(s_input_data, s_input_mask, s_ivector_data, s_target_data, s_target_mask)
        else:
            predict_output = predict_fn(s_input_data, s_input_mask, s_target_data, s_target_mask)

        ce_frame_sum, pred_idx = predict_output

        _, n_target_seq = target_data.shape
        
        pred_idx = pred_idx.repeat(skip, axis=1)
        pred_idx = pred_idx[:,:n_target_seq,:]        
