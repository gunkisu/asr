...
if floatX == 'float32':
    ...
    INF = numpy.float64(numpy.inf)

def ctc_strip(sequence, blank_symbol=63):
    res = []
...
def get_targets(y, log_y_hat, y_mask, y_hat_mask):
    ...
    # since the max is subtracted later, inf - inf = nan would occur
    max_marg = T.switch(T.le(max_marg, -INF), 0, max_marg)
    log_Z = T.log(T.exp(marginals - max_marg[:, :, None]).sum(2))
    log_Z = log_Z + max_marg
    log_Z = T.switch(T.le(log_Z, -INF), 0, log_Z)
    targets = _labeling_batch_to_class_batch(blanked_y,
                                             T.exp(marginals - log_Z[:, :, None]),
                                             num_classes + 1)
    return targets
...
def _log_dot_matrix(x, z):
    ...
    return T.switch(T.isnan(out), -INF, out)
