...
def get_feat_stream(path, which_set, batch_size=1, 
...
def get_uttid_stream(path, which_set, batch_size=1):
    wsj_dataset = H5PYDataset(path, which_sets=(which_set, ))
    iterator_scheme = SequentialScheme(examples=wsj_dataset.num_examples, 
        batch_size=batch_size)
    base_stream = DataStream(dataset=wsj_dataset,
...
def get_datastream(path, which_set, batch_size=1, norm_path=None, 
...
def create_ivector_test_datastream_with_targets(path, which_set, batch_size=1, delay=0):
    wsj_dataset = H5PYDataset(path, which_sets=(which_set, ))
    iterator_scheme = SequentialScheme(batch_size=batch_size, examples=wsj_dataset.num_examples)
    ...
    fs = FilterSources(data_stream=fs, sources=['features', 'ivectors', 'targets'])
    return Padding(fs)
