...
from libs.lasagne_libs.recurrent_layers import (LSTMLayer,
                                                ProjectLSTMLayer,
                                                FixedLayerNormProjectLSTMLayer,
                                                ...
                                                FixedMaxInputCondLayerNormProjectLSTMLayer)
...
def deep_projection_ln_lstm_model_fix(input_var,
                                      mask_var,
                                      num_inputs,
                                      num_outputs,
                                      num_layers,
...
def deep_projection_cond_ln_model_fix(input_var,
                                      mask_var,
                                      num_inputs,
                                      num_outputs,
                                      num_layers,
    ...
    for l  in range(num_layers):
        ...
        if l<num_conds:
            # forward
            fwd_feat_layer = FixedCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                mask_input=mask_layer,
                                                                num_units=num_units,
                                                                num_factors=num_factors,
                                                                grad_clipping=grad_clipping,
            ...
            # backward
            bwd_feat_layer = FixedCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                mask_input=mask_layer,
                                                                num_units=num_units,
                                                                num_factors=num_factors,
                                                                grad_clipping=grad_clipping,
...
def deep_projection_ivector_ln_model_fix(input_var,
                                         cond_var,
                                         mask_var,
                                         num_inputs,
                                         num_outputs,
    ...
    for l  in range(num_layers):
        ...
        if l<num_conds:
            # forward
            fwd_feat_layer = FixedInputCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                     cond_input=ivec_layer,
                                                                     mask_input=mask_layer,
                                                                     num_units=num_units,
                                                                     num_factors=num_factors,
            ...
            # backward
            bwd_feat_layer = FixedInputCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                     cond_input=ivec_layer,
                                                                     mask_input=mask_layer,
                                                                     num_units=num_units,
                                                                     num_factors=num_factors,
...
def deep_projection_max_cond_ln_model_fix(input_var,
                                          mask_var,
                                          num_inputs,
                                          num_outputs,
                                          num_layers,
    ...
    for l  in range(num_layers):
        ...
        if l<num_conds:
            # forward
            fwd_feat_layer = FixedMaxCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                   mask_input=mask_layer,
                                                                   num_units=num_units,
                                                                   num_factors=num_factors,
                                                                   grad_clipping=grad_clipping,
            ...
            # backward
            bwd_feat_layer = FixedMaxCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                   mask_input=mask_layer,
                                                                   num_units=num_units,
                                                                   num_factors=num_factors,
                                                                   grad_clipping=grad_clipping,
...
def deep_projection_max_ivector_ln_model_fix(input_var,
                                             cond_var,
                                             mask_var,
                                             num_inputs,
                                             num_outputs,
    ...
    for l  in range(num_layers):
        ...
        if l<num_conds:
            # forward
            fwd_feat_layer = FixedMaxInputCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                        cond_input=ivec_layer,
                                                                        mask_input=mask_layer,
                                                                        num_units=num_units,
                                                                        num_factors=num_factors,
            ...
            # backward
            bwd_feat_layer = FixedMaxInputCondLayerNormProjectLSTMLayer(incoming=prev_input_layer,
                                                                        cond_input=ivec_layer,
                                                                        mask_input=mask_layer,
                                                                        num_units=num_units,
                                                                        num_factors=num_factors,
