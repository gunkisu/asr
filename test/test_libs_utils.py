...
class UtilsTest(unittest.TestCase):
    ...
    def testSkipFamesFixed(self):
        every_n = 3
        new_seq = list(utils.skip_frames_fixed(self.a_seq, every_n))
        answer = [[[np.array([[1, 2],
       [7, 8]])]], [[np.array([[ 3,  4],
       [ 9, 10]])]], [[np.array([[5, 6]])]]]

        for sub_seq, sub_seq_answer in zip(new_seq[0], answer[0]):
            self.assertTrue(np.allclose(sub_seq[0], sub_seq_answer[0]))
