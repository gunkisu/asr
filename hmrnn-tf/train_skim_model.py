...
class TrainSkimModel(object):
  ...
  def __call__(self):
    ...
    train_G, sample_G = self._build_graph(self.FLAGS)
    train_ml_cost = tf.reduce_mean(train_G.ml_cost)
    train_rl_cost = tf.reduce_mean(train_G.rl_cost)
    tf.add_to_collection('losses', train_ml_cost)
    tf.add_to_collection('losses', train_rl_cost)

    # Set weight decay regularizer
    ...
    ml_opt_func = tf.train.AdamOptimizer(learning_rate=self.FLAGS.ml_learning_rate,
                                         beta1=0.9,
                                         beta2=0.99)
    rl_opt_func = tf.train.GradientDescentOptimizer(learning_rate=self.FLAGS.rl_learning_rate)

    if not self.FLAGS.grad_clip:
      # Without clipping
      t_step = opt_func.minimize(loss=_cost,
                                 global_step=global_step)
    else:
      # Apply gradient clipping using global norm
      tvars = tf.trainable_variables()
      grads, _ = tf.clip_by_global_norm(tf.gradients(ys=_cost,
                                                     xs=tvars,
                                                     aggregation_method=2),
                                        clip_norm=1.0)
      t_step = opt_func.apply_gradients(grads_and_vars=zip(grads, tvars),
                                        global_step=global_step)
